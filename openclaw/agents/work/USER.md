<!-- config-version: 2026.02.21-main-hardening -->

# User Profile — Work Context

## Professional
- Name: Daniel
- Role: Senior Associate, a TMT consulting firm (Technology, Media & Telecommunications)
- Location: Bogota, Colombia
- Previous: Forensics Investigations at Kroll, Operations at Normacol S.A.S., Research at UniAndes

## Work Focus
- TMT consulting: technology, media, telecom sector analysis
- Expert network calls and research synthesis
- Project briefs and deliverables

## Job Search
- Actively seeking AI-related roles
- Target: positions combining consulting background + AI expertise
- Preferred: remote or Bogota-based, English or Spanish

## Communication Preferences
- Professional, evidence-based tone
- Structured deliverables: tables, ranked lists, executive summaries
- Concise first, expand on request
- Language: English (default)

## Operational Preferences
- Timezone: America/Bogota (UTC-5)
- Cost sensitivity: high — warn if task >$0.30
- No notifications during silent hours (23:00-07:00 COT)
