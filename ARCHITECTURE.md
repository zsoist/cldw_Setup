...
| jobs.json | Cron job definitions | 2 jobs (both currently DISABLED) |
...
| docs/security/ | Access boundaries, hardening, secrets rotation |
| docs/playbooks/ | Operational playbooks |
| docs/templates/ | Output format templates |
...
| /root/.openclaw/cron/jobs.json | Live cron jobs (2 jobs) | sentinel:systemd-journal (640) | Both DISABLED |
...
| /opt/sentinel/*.py | Live Sentinel source | sentinel:sentinel (640) | Synced from repo |
| /opt/sentinel/__pycache__/*.pyc | Python bytecache | sentinel:sentinel (640) | MUST be sentinel-owned |
...
| /var/log/sentinel/ | Sentinel logs + cost data | sentinel:sentinel | api-usage.jsonl, api-cost-summary.json, vps-cost-cache.json |
...
| Job | Model | Timeout | Schedule (COT) | Status |
|-----|-------|---------|---------------|--------|
| news-brief-ai | openai-codex/gpt-5.3-codex | 120s | 07:10 daily | DISABLED |
| news-brief-enb | openai-codex/gpt-5.3-codex | 120s | 07:00 daily | DISABLED |
...
1. **File ownership after Edit**: /root/.openclaw/* -> chown sentinel:systemd-journal; /opt/sentinel/*.py AND __pycache__/*.pyc -> chown sentinel:sentinel
...
11. **Codex thinkingDefault**: "off" (model reasons internally regardless)
12. **Docker restart policy**: on-failure:5 (prevents restart spirals)
13. **HTTP health probe**: must use container exec_run(), NOT host curl (gateway resets host connections)
14. **.pyc ownership**: root-owned .pyc with 640 = sentinel can't import module (other=---). Always chown after editing.
...
repo sentinel/tests/        --test-->  104 tests (mocked, zero API cost)
...
| Config reload | `/root/.openclaw/reload-config.sh` (validates + fixes perms + SIGUSR1) | docker logs for errors | No success log |
| Sentinel | `systemctl restart sentinel` | `journalctl -u sentinel -f` or `/var/log/sentinel/sentinel.log` | `systemctl status sentinel` |
...
| JR Scheduler | `curl -X POST localhost:8080/api/v1/scheduler/{resume,pause}` | `docker logs job-radar-agent` | `curl localhost:8080/api/v1/scheduler/status` |
